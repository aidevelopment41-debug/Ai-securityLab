...
useEffect(() => {
  ...
  // FIXED: updateLogoSize inside its own effect
  useEffect(() => {
    ...
    const buildGrid = (w, h) => {
      ...
      for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
          ...
          const r0 = 200;
          let field = r < r0 ? (255 / r0) * r : 255 - Math.min(255, (r - r0) / 1.5);
    ...
    const draw = () => {
      ...
      particles.forEach((p) => {
        ...
        for (let i = 1; i < p.trail.length; i++) {
          ...
          ctx.moveTo(p.trail[i - 1].x + xC, p.trail[i - 1].y + yC);
          ctx.lineTo(p.trail[i].x + xC, p.trail[i].y + yC);
          ctx.stroke();
        }
      });
